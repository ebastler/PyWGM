...
def runtopdbrscan(substrateHeight, botDBR1Height, botDBR2Height, cavityHeight, topDBR1Height, topDBR2Height, capHeight, minPillarRadius, absWallW, minTopDBRpairs, maxTopDBRpairs, botDBRpairs, unEtchedBotDBRpairs, minApertureSize, apertureThickness, apertureSpacing, apertureEnable, permApertureOx, permApertureNonOx, permSubstrate, permBotDBR1, permBotDBR2, permCavity, permTopDBR1, permTopDBR2, permCap, permAbsWall, simFEMdeg, simPrecision, simFieldRes, modeNumber, jcmMaxThreads, startWL, stopWL, targetWL):
    ...
    if not os.path.exists('res_topdbrscan'):
        os.makedirs('res_topdbrscan')
        print('Creating directory \"res_topdbrscan\"')
    else:
        print('Directory \"res_topdbrscan\" already exists. Skipping creation.')
    if not os.path.exists('res_topdbrscan/fields'):
        os.makedirs('res_topdbrscan/fields')
        print('Creating directory \"res_topdbrscan/fields\"')
    else:
        print('Directory \"res_topdbrscan/fields\" already exists. Skipping creation.')

    resPath = 'res_topdbrscan/topdbrscan.csv'
    ...
    with open(resPath, mode='w', newline='') as csvfile:
        ...
        f.writeheader()
        # Iterate from lowest to highest amount of desired top DBR pairs, start JCM solver + plot results for each
        for iterTopDBRpairs in range(minTopDBRpairs, maxTopDBRpairs+1, 1):
            ...
            airHeight =  300 + substrateHeight + botDBR1Height + botDBRpairs * (botDBR1Height + botDBR2Height) + cavityHeight + topDBR1Height + iterTopDBRpairs * (topDBR1Height + topDBR2Height) + capHeight
            # absWallH sets the height of the absorbing layer placed on the outside of the pillar - must be same height as etched pillar
            absWallH = capHeight + topDBR1Height + iterTopDBRpairs * (topDBR1Height + topDBR2Height) + cavityHeight + (botDBRpairs - unEtchedBotDBRpairs) * (botDBR1Height + botDBR2Height)
                    ...
                    try:
                        ...
                        for i in range(0, iterModeNumber):
                            ...
                            if JCMresults[2][i] > 0:
                                ...
                                # Set plot title
                                plotTitle = 'R = %d nm, Aperture = %d nm, Nphi = %d, WL = %.2f nm, Q = %.2E' %(minPillarRadius, minApertureSize, iternPhi, JCMresults[0][i], JCMresults[1][i])
                                plotPath = 'res_topdbrscan/fields/{}_topDBRpairs_{}_nm_{}nPhi_{}_{:.2f}nm.png'.format(minPillarRadius, iterTopDBRpairs, iternPhi, fields[0], JCMresults[0][i])
                                pyWGM_plot.plotfields(plotTitle, plotPath, JCMresults[5], JCMresults[6], JCMresults[7], fields[0], fields[1], fields[2], botDBR1Height, botDBR2Height, cavityHeight,    topDBR1Height, topDBR2Height, capHeight, iterTopDBRpairs, botDBRpairs, unEtchedBotDBRpairs, minPillarRadius, iterSubstrateRadius, apertureRadius, aperturePosition,    apertureEnable, apertureThickness)

                                # Print saved file info to the console
                                print('Plot for R = {} nm, top DBR pairs = {} nm, WL = {:.2f} nm saved. Mode Volume: {:.0f} nm³, purcell Factor = {:.1f}, Distance wall to mode max = {:.1f} nm'.   format(minPillarRadius, iterTopDBRpairs, JCMresults[0][i], fields[4], fields[5], minPillarRadius-fields[3]))
                                # Save all computed values for WGMs to the csv
                                f.writerow({'Radius [nm]': minPillarRadius, 'Top DBR pairs': iterTopDBRpairs, 'unetched DBR pairs': unEtchedBotDBRpairs, 'Oxide width [nm]':minApertureSize, 'nPhi':    iternPhi, 'Polarisation': fields[0], 'Wavelength [nm]': JCMresults[0][i], 'Q-Factor': JCMresults[1][i], 'Mode Volume [nm^3]': fields[4], 'Purcell Factor': fields  [5], 'Beta Factor': fields[6], 'Distance wall to mode max [nm]': minPillarRadius-fields[3], 'energyIntStructure': JCMresults[9][i], 'energyIntAir': JCMresults[10]    [i] })
    ...
    pyWGM_plot.plotresults(resPath, plotPath, 'topdbr', 'singlePoint', targetWL)
