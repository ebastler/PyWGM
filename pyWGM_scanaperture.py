...
def runaperturescan(substrateHeight, botDBR1Height, botDBR2Height, cavityHeight, topDBR1Height, topDBR2Height, capHeight, minPillarRadius, absWallW, topDBRpairs, botDBRpairs, unEtchedBotDBRpairs, minApertureSize, maxApertureSize, apertureSizeStepping, apertureThickness, apertureSpacing, apertureEnable, permApertureOx, permApertureNonOx, permSubstrate, permBotDBR1, permBotDBR2, permCavity, permTopDBR1, permTopDBR2, permCap, permAbsWall, simFEMdeg, simPrecision, simFieldRes, modeNumber, jcmMaxThreads, startWL, stopWL, targetWL):
    ...
    if not os.path.exists('res_aperturescan'):
        os.makedirs('res_aperturescan')
        print('Creating directory \"res_aperturescan\"')
    else:
        print('Directory \"res_aperturescan\" already exists. Skipping creation.')
    if not os.path.exists('res_aperturescan/fields'):
        os.makedirs('res_aperturescan/fields')
        print('Creating directory \"res_aperturescan/fields\"')
    else:
        print('Directory \"res_aperturescan/fields\" already exists. Skipping creation.')

    resPath = 'res_aperturescan/aperturescan.csv'
    ...
    with open(resPath, mode='w', newline='') as csvfile:
        ...
        for iterApertureSize in itertools.chain([0], range(minApertureSize, maxApertureSize+apertureSizeStepping, apertureSizeStepping)):
            ...
            iterApertureRadius = minPillarRadius - iterApertureSize
            # Disable the AlAs/aperture layer if size 0, but make the actual structure non-0 in size to avoid issues with jcm parsing the structure file, since even disabled polygons are parsed and checked for intersecting outlines
            if iterApertureSize == 0:
                apertureEnable = False
                iterApertureRadius = minPillarRadius - 10
            else:
                apertureEnable = True
            ...
            for iternPhi in range(nPhi, nPhi+20):
                ...
                while True:
                    ...
                    print('Starting JCM solver for aperture scan - oxide width = {} nm, nPhi = {}'.format(iterApertureSize, iternPhi))
                    try:
                        ...
                        for i in range(0, iterModeNumber):
                            ...
                            if JCMresults[2][i] > 0:
                                ...
                                # Set plot title
                                plotTitle = 'R = %d nm, Aperture = %d nm, Nphi = %d, WL = %.2f nm, Q = %.2E' %(minPillarRadius, iterApertureSize, iternPhi, JCMresults[0][i], JCMresults[1][i])
                                plotPath = 'res_aperturescan/fields/{}nm_ap_{}_nm_{}nPhi_{}_{:.2f}nm.png'.format(minPillarRadius, iterApertureSize, iternPhi, fields[0], JCMresults[0][i])
                                pyWGM_plot.plotfields(plotTitle, plotPath, JCMresults[5], JCMresults[6], JCMresults[7], fields[0], fields[1], fields[2], botDBR1Height, botDBR2Height, cavityHeight,    topDBR1Height, topDBR2Height, capHeight, topDBRpairs, botDBRpairs, unEtchedBotDBRpairs, minPillarRadius, iterSubstrateRadius, iterApertureRadius, aperturePosition,    apertureEnable, apertureThickness)

                                # Print saved file info to the console
                                print('Plot for R = {} nm, oxide width = {} nm, WL = {:.2f} nm saved. Mode Volume: {:.0f} nm³, purcell Factor = {:.1f}, Distance wall to mode max = {:.1f} nm'.format   (minPillarRadius, iterApertureSize, JCMresults[0][i], fields[4], fields[5], minPillarRadius-fields[3]))
                                # Save all computed values for WGMs to the csv
                                f.writerow({'Radius [nm]': minPillarRadius, 'Top DBR pairs': topDBRpairs, 'unetched DBR pairs': unEtchedBotDBRpairs, 'Oxide width [nm]':iterApertureSize, 'nPhi':   iternPhi, 'Polarisation': fields[0], 'Wavelength [nm]': JCMresults[0][i], 'Q-Factor': JCMresults[1][i], 'Mode Volume [nm^3]': fields[4], 'Purcell Factor': fields [5], 'Beta Factor': fields[6], 'Distance wall to mode max [nm]': minPillarRadius-fields[3], 'energyIntStructure': JCMresults[9][i], 'energyIntAir': JCMresults[10]   [i] })
    ...
    pyWGM_plot.plotresults(resPath, plotPath, 'aperture', 'singlePoint', targetWL)
